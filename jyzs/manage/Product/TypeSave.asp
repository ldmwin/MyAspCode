...
'   if p_id =0 then
'   sqlstr = "SET NOCOUNT ON insert into Product_Type(Type_Name,Parent_ID,Type_Level,Adder_ID,Add_Time,Status,Show_Order) values('"& Type_Name &"'," & p_id & ",1," & User.UserID & ",getdate(),1," & Show_Order & ") SELECT SCOPE_IDENTITY() SET NOCOUNT off"
'	else
'	sqlstr = "DECLARE @level int select @level=Type_Level+1 from Product_Type where id=" & p_id & " SET NOCOUNT ON insert into Product_Type(Type_Name,Parent_ID,Type_Level,Adder_ID,Add_Time,Status,Show_Order) values('"& Type_Name &"'," & p_id & ",@level," & User.UserID & ",getdate(),1," & Show_Order & ") SELECT SCOPE_IDENTITY() SET NOCOUNT off"
'	end if
'	
'	
'	rsid = db.AddRecordBySql(sqlstr)
	...
	if p_id =0 then
		...
		Set rs_Type = db.getRecordBySQL("select Type_Level from Product_Type where id=" & p_id)
		...
		call db.AddRecordByRS("addnew","Product_Type")
...
'插入后的获取自动编号
   ...
   sqlstr = "update Product_Type set Type_Name='"& Type_Name &"',Show_Order=" & Show_Order & " where id=" & c_id
...
elseif action="up" or action="down" or action="cancel" or action="recommend" or action="unrecommend" then
	...
	p_id=cint(p_id) 
		...
		select case action
		case "up"
			sqlstr="update Product_Type set status=1 where id in (" & Infos_id & ") and status=0"
			db.DoExecute(SqlStr)
		case "down"
			sqlstr="update Product_Type set status=0 where id in (" & Infos_id & ") and status=1"
			db.DoExecute(SqlStr)
		case "cancel"
			sqlstr="update Product_Type set status=4 where id in (" & Infos_id & ")"
			db.DoExecute(SqlStr)
