...
'接收查询数据
Dim Keyword,RsStatus,SearchStartTime,SearchEndTime,ProductType,sqlstr
'Dim Proname:Proname="Information_List"
Dim Current_page
Dim DataRs
Dim SubmitUrl:SubmitUrl = "ProductManage.asp"
...
if Request.ServerVariables("HTTP_METHOD")="POST" then 
	...
	sqlstr = "select *,(select type_name from Product_Type where Product_Type.id=Products.Product_Type) as ProductTypeshow,(select UserName from Sys_User where Sys_User.id= Products.Adder_ID) as Adder from Products where  " & TranslateDateTime(SearchStartTime,0) & " <=Add_Time and Add_Time<=" & TranslateDateTime(SearchEndTime,1) & " and status<>4"
	...
	if ProductType <>"0" then
		sqlstr = sqlstr & " and Product_Type = " & ProductType
	end if		
	...
	sqlstr = "select *,(select type_name from Product_Type where Product_Type.id=Products.Product_Type) as ProductTypeshow,(select UserName from Sys_User where Sys_User.id= Products.Adder_ID) as Adder from Products where  " & TranslateDateTime(formatdatetime(Now()-30,2),0) & " <=Add_Time and Add_Time<=" & TranslateDateTime(formatdatetime(Now(),2),1) & " and status<>4 order by show_order desc,id desc"
...
Sub OperateSearchItem()
	...
	session("back_ProductType") = ProductType
...
<table border="0" cellspacing="1" cellpadding="5" height="1" align="center" width="100%">
		<tr class="tr2">
			<td colspan="6">
			<input type="button" name="add" class="button" value="新建" style="margin-left:20px;" onClick="GotoUrl('ProductAdd.asp');"/>
			<input type="button" name="up" class="button" value="上线" style="margin-left:20px;" onClick="GoToUrl_MorePrm('ProductSave.asp?action=up','确认上线？');"/>
			<input type="button" name="down" class="button" value="下线" style="margin-left:20px;" onClick="GoToUrl_MorePrm('ProductSave.asp?action=down','确认下线？');"/>
			<input name="cancel" type="button" class="button" id="cancel" style="margin-left:20px;" value="作废" onClick="GoToUrl_MorePrm('ProductSave.asp?action=cancel','确认作废？');"/>
		...
		<tr class="tr1">
			...
			<option value="0" <%=object_selected(cint(ProductType),0)%>>所有</option>
			<%
			Set rs = db.getRecordBySQL("select * from Product_Type where status=1 and Type_Level=2 order by show_order desc,id desc")
			if not(rs.eof and rs.bof) then
			do until rs.eof
			response.Write("<option value=""" & rs("id") & """ " & object_selected(cint(ProductType),cint(rs("id"))) & ">" & rs("Type_Name") & "</option>")
			rs.movenext
			loop
			end if
			db.C(rs)
...
<table border="0" cellspacing="1" cellpadding="5" height="1" align="center" width="100%">
	<tr>
		<th colspan="10" style="text-align:center;">产品列表</th>
	</tr>
	<tr class="tr2" align="center">
		<td width="5%"><input name="chk_all" type="checkbox" id="chk_all" onClick="SelectAll();" /></td>
		<td width="5%"><b>ID</b></td>
		<td width="23%"><strong>品名</strong></td>
		<td width="9%"><strong>产品类型</strong></td>
...
%>
	<tr class="tr1" onMouseOver="this.style.backgroundColor='#C4D8ED'" onMouseOut ="this.style.backgroundColor='#F1F3F5'" >
		...
		<td><a target="_self" href="InfoView.asp?id=<%=DataRs("id")%>"><%=DataRs("Product_Name")%></a></td>
		<td align="center"><%=DataRs("ProductTypeshow")%></td>
		<td align="center"><%=StatusResult(1,DataRs("status"))%></td>
		<td align="center"><%=DataRs("Add_Time")%></td>
		<td align="center"><%=DataRs("Adder")%></td>
		<td align="center"><%=DataRs("Product_Hits")%></td>
		<td align="center"><%=DataRs("Show_Order")%></td>
		<td align="center"><%if DataRs("status")=0 or DataRs("status")=2 then%>
						<input type="button" name="edit" class="button" value="编辑" style="margin-left:5px;" onClick="GotoUrl('ProductEdit.asp?id=<%=DataRs("id")%>');"/>
						<%end if%>&nbsp;
	  </td>
	</tr>
<%
