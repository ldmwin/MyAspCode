...
<table border="0" cellspacing="1" cellpadding="5" height="1" align="center" width="100%">
		...
		sqlstr = "select *,(select count(*) from Information_Type a where a.status=1 and a.Parent_id=Information_Type.id) as upchildfloornum,(select count(*) from Information_Type b where b.status=0 and b.parent_id=Information_Type.id) as downchildfloornum from Information_Type where parent_id=0 and status<>4 order by show_order desc,id desc"
