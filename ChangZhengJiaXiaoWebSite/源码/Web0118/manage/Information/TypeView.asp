...
Dim rs_Type : Set rs_Type = db.getRecordBySQL("select *,(select Type_Name from Information_Type  a where a.id=c.Parent_ID) as P_name from Information_Type c where status<>4 and id = " & Infos_id)
...
<body>
  ...
  <table border="0" cellspacing="1" cellpadding="5" height="1" align="center" width="100%">
		...
		<%Dim rs_childType
	Set rs_childType = db.getRecordBySQL("select *,(select Type_name from Information_Type a where a.Parent_ID=c.id) as P_name from Information_Type c where status<>4 and Parent_id=" & rs_Type("id") & " order by show_order desc,id desc") 
