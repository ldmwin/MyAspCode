...
(function($){
    ...
    $.fn.wBox = function(options){
        ...
        function handleClick(){
			...
			if (_this.YQ.requestType && $.inArray(_this.YQ.requestType, ['iframe', 'iframe_refresh', 'iframe_resubmit', 'iframe_video' , 'ajax','img'])!=-1) {
				...
				if (_this.YQ.requestType === "img") {
					...
					if (_this.YQ.requestType === "ajax") {
						...
						//alert(_this.YQ.requestType);
						//alert(_this.YQ.target);
						var pagesrc = _this.YQ.target;
						
						if (_this.YQ.requestType === "iframe_video") {
							var filesource = document.getElementById("Video_File").value;
							if(filesource != ""){
							pagesrc = _this.YQ.target + "?filesource=" + filesource;
							}
						}
						//else{
							//pagesrc = _this.YQ.target;	
						//}
						
						ifr = $("<iframe name='wBoxIframe' style='width:" + _this.YQ.iframeWH.width + "px;height:" + _this.YQ.iframeWH.height + "px;' scrolling='auto' frameborder='0' src='" + pagesrc + "'></iframe>");
        ...
        function setPosition(){
            ...
            //$h = Math.max($h, $wh);
			$h =(document.body.offsetHeight>document.body.scrollHeight)?'100%':document.body.scrollHeight;/*修正iframe中遮罩层不能满窗口显示问题，by Mr.Lion 2011.7.1*/
            B.height($h).width($win.width())   
			//B.height(100).width($win.width())
        }
        /*
         * 计算wBox的位置
         * @param {number} w 宽度
        ...
        this.close=function (){
			...
			//alert("no");
			//window.location.reload(); /*关闭窗口时页面刷新*/
			if (_this.YQ.requestType === "iframe_refresh"){
			//alert("yes");
			window.location.reload();
			//location.href = location.href;
			}else if(_this.YQ.requestType === "iframe_resubmit"){
