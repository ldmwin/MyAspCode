...
Dim Proname:Proname="Advertisement_Marquee_List"
Dim Current_page
Dim DataRs:Set DataRs = Server.CreateObject("adodb.recordset")
Dim SubmitUrl:SubmitUrl = "ADMarqueeManage.asp"
...
<table border="0" cellspacing="1" cellpadding="5" height="1" align="center" width="100%">
		<tr class="tr2">
			<td colspan="6"><input name="return" type="button" class="button" id="return" onClick="GotoUrl('ADMarqueeAdd.asp?ad_id=<%=AD_ID%>');" value="新建项目" style="margin-left:20px;"/>
			<input type="button" name="up" class="button" value="上线" style="margin-left:20px;" onClick="GoToUrl_MorePrm('ADMarqueeSave.asp?action=up','确认上线？');"/>
			<input type="button" name="down" class="button" value="下线" style="margin-left:20px;" onClick="GoToUrl_MorePrm('ADMarqueeSave.asp?action=down','确认下线？');"/>
			<input name="cancel" type="button" class="button" id="cancel" style="margin-left:20px;" value="作废" onClick="GoToUrl_MorePrm('ADMarqueeSave.asp?action=cancel','确认作废？');"/>
...
<%end if%>&nbsp;</td>
		...
		<td align="center"><%if DataRs("status")=0 or DataRs("status")=2 then%>
						<input type="button" name="edit" class="button" value="编辑" style="margin-left:5px;" onClick="GotoUrl('ADMarqueeEdit.asp?id=<%=DataRs("id")%>');"/>
						<%end if%>&nbsp;&nbsp;</td>
	</tr>
<%
	DataRs.movenext()
