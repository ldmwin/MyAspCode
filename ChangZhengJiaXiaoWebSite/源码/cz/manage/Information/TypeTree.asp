...
Function BuildXMLStr(pid,str) '递归类别及其子类别存入字符串
	...
	Set rs_menutree = db.getRecordBySQL("select m.*,(select count(*) from Information_Type s where s.Parent_id=m.id and s.status<>4) as childnodenum from Information_Type m where m.status<>4 and m.parent_id=" & pid & " order by m.show_order desc,m.id desc") 
	i  = 0
