...
'接收查询数据
Dim Keyword,RsStatus,SearchStartTime,SearchEndTime,InfoType,sqlstr
'Dim Proname:Proname="Information_List"
Dim Current_page
Dim DataRs
Dim SubmitUrl:SubmitUrl = "InfoManage.asp"
...
if Request.ServerVariables("HTTP_METHOD")="POST" then 
	...
	sqlstr = "select *,(select type_name from Information_Type where Information_Type.id=Informations.Info_Type) as infotypeshow,(select UserName from Sys_User where Sys_User.id= Informations.Adder_ID) as Adder from Informations where  " & TranslateDateTime(SearchStartTime,0) & " <=Add_Time and Add_Time<=" & TranslateDateTime(SearchEndTime,1) & " and status<>4"
	...
	if InfoType <>"0" then
		sqlstr = sqlstr & " and Info_Type = " & InfoType
	end if		
	...
	sqlstr = "select *,(select type_name from Information_Type where Information_Type.id=Informations.Info_Type) as infotypeshow,(select UserName from Sys_User where Sys_User.id= Informations.Adder_ID) as Adder from Informations where  " & TranslateDateTime(formatdatetime(Now()-30,2),0) & " <=Add_Time and Add_Time<=" & TranslateDateTime(formatdatetime(Now(),2),1) & " and status<>4 order by show_order desc,id desc"
...
Sub OperateSearchItem()
	...
	session("back_InfoType") = InfoType
...
<table border="0" cellspacing="1" cellpadding="5" height="1" align="center" width="100%">
		<tr class="tr2">
			<td colspan="6">
			<input type="button" name="add" class="button" value="新建" style="margin-left:20px;" onClick="GotoUrl('InfoAdd.asp');"/>
			<input type="button" name="up" class="button" value="上线" style="margin-left:20px;" onClick="GoToUrl_MorePrm('InfoSave.asp?action=up','确认上线？');"/>
			<input type="button" name="down" class="button" value="下线" style="margin-left:20px;" onClick="GoToUrl_MorePrm('InfoSave.asp?action=down','确认下线？');"/>
			<input name="cancel" type="button" class="button" id="cancel" style="margin-left:20px;" value="作废" onClick="GoToUrl_MorePrm('InfoSave.asp?action=cancel','确认作废？');"/>
		...
		<tr class="tr1">
			...
			<option value="0" <%=object_selected(cint(InfoType),0)%>>所有</option>
			<%
			Set rs = db.getRecordBySQL("select * from Information_Type where status=1 and Type_Level=2 order by show_order desc,id desc")
			if not(rs.eof and rs.bof) then
			do until rs.eof
			response.Write("<option value=""" & rs("id") & """ " & object_selected(cint(InfoType),cint(rs("id"))) & ">" & rs("Type_Name") & "</option>")
			rs.movenext
			loop
			end if
			db.C(rs)
...
<table border="0" cellspacing="1" cellpadding="5" height="1" align="center" width="100%">
	<tr>
		<th colspan="10" style="text-align:center;">新闻列表</th>
	</tr>
...
%>
	<tr class="tr1" onMouseOver="this.style.backgroundColor='#C4D8ED'" onMouseOut ="this.style.backgroundColor='#F1F3F5'" >
		...
		<td><a target="_self" href="InfoView.asp?id=<%=DataRs("id")%>"><%=DataRs("Title")%></a></td>
		<td align="center"><%=DataRs("InfoTypeshow")%></td>
		<td align="center"><%=StatusResult(1,DataRs("status"))%></td>
		<td align="center"><%=DataRs("Add_Time")%></td>
		<td align="center"><%=DataRs("Adder")%></td>
		<td align="center"><%=DataRs("Info_Hits")%></td>
		<td align="center"><%=DataRs("Show_Order")%></td>
		<td align="center"><%if DataRs("status")=0 or DataRs("status")=2 then%>
						<input type="button" name="edit" class="button" value="编辑" style="margin-left:5px;" onClick="GotoUrl('InfoEdit.asp?id=<%=DataRs("id")%>');"/>
						<%end if%>&nbsp;
	  </td>
	</tr>
<%
