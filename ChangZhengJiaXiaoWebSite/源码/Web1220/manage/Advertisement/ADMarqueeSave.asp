...
<body>
<%sub return_to_list()%>
<form action="ADMarqueeManage.asp" method=post name="frm_page">
...
'插入后的获取自动编号
   ...
   sql = "SET NOCOUNT ON insert into Advertisement_Marquee(Item_Name,Show_Start_Time,Show_End_Time,Advertisement_ID,Adder_ID,Add_Time,Status,Show_Order,Link,Target,Pic_Url,Pic_View_Url) values('"& Item_Name &"'," & Show_Start_Time & "," & Show_End_Time & "," & AD_ID & "," & User.UserID & ",getdate(),0," & Show_Order & ",'" & Link & "','" & Target & "','" & Pic_Url & "','" & Pic_View_Url & "') SELECT SCOPE_IDENTITY() SET NOCOUNT off"
   ...
   'response.Write(sql)
    ...
    if cint(rsid) > 0 then
	   ...
	   BackUrl = "ADMarqueeManage.asp?ad_id=" & AD_ID
	   Msg = "广告项目添加成功"
		%>
...
elseif action="edit" then
	Infos_id = request.form("Marquee_id")
	...
	SqlStr="update Advertisement_Marquee set Item_Name ='" & Item_Name & "',Show_Start_Time=" & Show_Start_Time & ",Show_End_Time=" & Show_End_Time & ",Advertisement_ID='" & AD_ID & "',Target='" & Target & "',Link='" & Link & "',Pic_Url='" & Pic_Url & "',Pic_View_Url='" & Pic_View_Url & "',Show_Order=" & Show_Order & " where id=" & Infos_id
...
elseif action="up" or action="down" or action="cancel" or action="del" then
	...
	if i<>0 then
		...
		select case action
		case "up"
			sqlstr="update Advertisement_Marquee set status=1 where id in (" & info_id & ") and (status=0 or status=2)"
			'response.Write(sqlstr)
			'response.End()
			db.DoExecute(SqlStr)
		case "down"
			sqlstr="update Advertisement_Marquee set status=2 where id in (" & info_id & ") and status=1"
			db.DoExecute(SqlStr)
		case "cancel"
			sqlstr="update Advertisement_Marquee set status=4 where id in (" & info_id & ")"
			db.DoExecute(SqlStr)
