...
Dim rs_Banner : Set rs_Banner = db.getRecordBySQL("select *,(select Advertisement_name from Advertisement where Advertisement.id=Advertisement_Banner.Advertisement_ID) as Advertisement_name from Advertisement_Banner where status<>4 and id = " & Info_id)

if rs_Banner.eof or rs_Banner.bof then
	response.write "数据查询失败"
	response.End()
end if

picurl1= Config.ImgUrl() &"AD/" & rs_Banner("Pic_Url")
picurl2= Config.ImgUrl() &"AD/" & rs_Banner("Pic_View_Url")
%>
...
<body>
<form name="BannerEdit" method="post" action="ADBannerSave.asp">
	<table border="0" cellspacing="1" cellpadding="5" height="1" align="center" width="100%">
		<tr class="tr2">
			<td align="left" colspan="4">
				...
				<input type="button" name="return" class="button" value="返回列表" style="margin-left:20px;" onClick="GotoUrl('ADBannerManage.asp?AD_ID=<%=rs_Banner("Advertisement_ID")%>');"/><input type="button" name="refresh" value="刷新" class="button" style="margin-left:20px;"  onClick="Refresh('_self');"/>
				<input type="hidden" name="action" value="edit"><input type="hidden" name="Banner_ID" value="<%=rs_Banner("ID")%>"></td>
		</tr>
		<tr>
		...
		<tr class="tr1">
		  <td>对应页面广告：</td>
		  <td colspan="3"><%=rs_Banner("Advertisement_Name")%>&nbsp;<input type="hidden" name="AD_ID" value="<%=rs_Banner("Advertisement_ID")%>" /></td>
	  </tr>
		<tr class="tr2">
			<td width="20%">项目名称：</td>
			<td width="80%" colspan="3"><input name="Item_Name" type="text" id="Item_Name" size="50" value="<%=rs_Banner("Item_Name")%>"/>
			*</td>
		</tr>
		<tr class="tr1">
		  <td width="20%">展示开始时间：</td>
		  <td width="30%"><input type="text" name="Show_Start_Time" id="Show_Start_Time" onfocus="WdatePicker({dateFmt:'yyyy-M-d H:m:s'})" class="Wdate" value="<%=rs_Banner("Show_Start_Time")%>"/>
		    不设置为马上呈现</td>
		  <td width="20%">展示结束时间：</td>
		  <td width="30%"><input type="text" name="Show_End_Time" id="Show_End_Time" onfocus="WdatePicker({dateFmt:'yyyy-M-d H:m:s'})" class="Wdate" value="<%=rs_Banner("Show_End_Time")%>" />
		    不设置为永久呈现</td>
	  </tr>
		
		<tr class="tr2">
			<td>链接：</td>
			<td colspan="3" class="tr2"><input name="Link" type="text" id="Link" size="50" value="<%=rs_Banner("Link")%>"/></td>
      </tr>
		...
		<tr class="tr1">
		  ...
		  <td colspan="3"><label>
		    <select name="Target" id="Target">
		      <option value="_self" <%=object_selected(rs_Banner("Target"),"_self")%>>_self</option>
		      <option value="_blank" <%=object_selected(rs_Banner("Target"),"_blank")%>>_blank</option>
		      <option value="_parent" <%=object_selected(rs_Banner("Target"),"_parent")%>>_parent</option>
		      <option value="_top" <%=object_selected(rs_Banner("Target"),"_top")%>>_top</option>
	        </select>
		  </label> 
	      *</td>
	  </tr>
		...
		<tr class="tr2">
		  <td>图片：</td>
		  <td colspan="3"><input name="Pic_Url" type="text" id="Pic_Url" size="50" value="<%=rs_Banner("Pic_Url")%>"/>
		...
		<tr class="tr1">
		  <td>缩略图：</td>
		  <td colspan="3"><input name="Pic_View_Url" type="text" id="Pic_View_Url" size="50" value="<%=rs_Banner("Pic_View_Url")%>" />
		...
		<tr class="tr2">
		  <td>显示顺序</td>
		  <td colspan="3"><input name="Show_Order" type="text" id="Show_Order" size="10" value="<%=rs_Banner("Show_Order")%>"/>
*</td>
	  </tr>
  </table>
</form>
